...
`GET /reporting/reinvestigation-outcomes/benchmarks` (Phase 15) returns org-internal trailing baselines for the same analytics shape. Query params: `baseline_days` (range 7–365), `recent_days` (must be ≤ `baseline_days`), and optional `bureau`. When window params are omitted, dispute-settings defaults apply: per-bureau override when `bureau` is set (Phase 18), else org-wide pair (Phase 17), else platform `90` / `30`. Response includes `scope` (`organization`), `baseline_period` / `baseline`, `recent_period` / `recent`, and advisory `rate_deltas` (recent rate minus baseline rate for deletion/verification/correction/favorable/no_response). No cross-tenant data and no live bureau contact; `recent_days` > `baseline_days` returns `422`. As of Phase 16 the Enterprise reporting UI exposes an **Outcome benchmarks** tab for this endpoint; Phase 17 initializes the tab from org defaults; Phase 18 adds optional Equifax/Experian/TransUnion window overrides on `GET`/`PATCH /org-admin/dispute-settings` (`reinvestigation_benchmark_bureau_windows`).
