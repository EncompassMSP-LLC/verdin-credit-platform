...
`GET /reporting/reinvestigation-outcomes/benchmarks` (Phase 15) returns org-internal trailing baselines for the same analytics shape. Query params: `baseline_days` (range 7–365), `recent_days` (must be ≤ `baseline_days`), optional `bureau`, and optional `group_by=bureau` (Phase 18). When window params are omitted, dispute-settings defaults apply: per-bureau override when `bureau` is set (Phase 18), else org-wide pair (Phase 17), else platform `90` / `30`. Response includes `scope` (`organization`), `baseline_period` / `baseline`, `recent_period` / `recent`, advisory `rate_deltas`, and when `group_by=bureau` a `by_bureau` array of per-bureau baseline/recent analytics + deltas. No cross-tenant data and no live bureau contact; invalid `group_by` or `recent_days` > `baseline_days` returns `422`. As of Phase 16 the Enterprise reporting UI exposes an **Outcome benchmarks** tab; Phase 17 initializes from org defaults; Phase 18 adds per-bureau window overrides and always requests `group_by=bureau` for the per-bureau rates table.
