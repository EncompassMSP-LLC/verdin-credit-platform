...
| POST   | `/cases/{case_id}/resolutions/reresolve`                           | case_manager | Bulk enqueue entity resolve for documents with extracted metadata                                                                                               |
...
| GET    | `/cases/{case_id}/identity-theft/605b-packet.zip`                  | read_only    | Staff-mediated FCRA Â§605B block packet (bureau letters + readiness; optional `document_id` evidence exhibits; requires confirmed theft)                        |
| POST   | `/cases/{case_id}/credit-analysis/runs`                            | case_manager | Deterministic Lending Readiness Score™ compose + publish (advisory; no bureau I/O); blockers include tradeline + cross-bureau + high-severity Metro 2 (LRP-402) |
| GET    | `/cases/{case_id}/credit-analysis/runs`                            | read_only    | List credit-analysis run summaries for a case                                                                                                                   |
| GET    | `/cases/{case_id}/credit-analysis/runs/latest`                     | read_only    | Latest credit-analysis run (`404` if none)                                                                                                                      |
| GET    | `/cases/{case_id}/credit-analysis/runs/{run_id}`                   | read_only    | Replay a specific credit-analysis run                                                                                                                           |
| GET    | `/cases/{case_id}/credit-analysis/runs/{run_id}/export`            | read_only    | Export run as text or PDF (`?export_format=text\|pdf`); disclaimer leads every export; operator-gated, never auto-transmitted                                   |
| POST   | `/cases/{case_id}/consultation-pack/runs`                          | case_manager | Staff-gated consultation completed pack (draft artifacts; LRP-204); never auto-sends partner notification                                                       |
| GET    | `/cases/{case_id}/consultation-pack/runs`                          | read_only    | List consultation pack draft runs                                                                                                                               |
| GET    | `/cases/{case_id}/consultation-pack/runs/latest`                   | read_only    | Latest consultation pack (`404` if none)                                                                                                                        |
| GET    | `/cases/{case_id}/consultation-pack/runs/{run_id}`                 | read_only    | Replay a consultation pack run                                                                                                                                  |
| GET    | `/cases/{case_id}/consultation-pack/runs/{run_id}/export`          | read_only    | Export pack as text or ZIP (`?export_format=`); draft only; never auto-transmitted                                                                              |
| POST   | `/cases/{case_id}/identity-theft/605b-readiness-runs`              | case_manager | Operator-gated Â§605B submission-readiness audit run (records outcome; never submits to a bureau)                                                               |
| GET    | `/cases/{case_id}/identity-theft/605b-readiness-runs/latest`       | read_only    | Latest recorded Â§605B submission-readiness audit run (`404` if none)                                                                                           |
...
| GET    | `/cases/{case_id}/issue-explainability`                            | read_only    | Plain-language issue cards with impact categories, evidence recommendations, and vault associations (LRP-208/208A; no score-point promises)                     |
| GET    | `/cases/{case_id}/issue-evidence-links`                            | read_only    | List staff-mediated document↔issue vault associations (LRP-208A; optional `source_id`)                                                                          |
| POST   | `/cases/{case_id}/issue-evidence-links`                            | case_manager | Associate a case vault document with an explainability `source_id` (LRP-208A; never auto-files)                                                                 |
| DELETE | `/cases/{case_id}/issue-evidence-links/{link_id}`                  | case_manager | Soft-delete a vault document↔issue association (LRP-208A)                                                                                                       |
| GET    | `/cases/{case_id}/letter-drafts`                                   | read_only    | List Intelligent Letter Draft Builder drafts + template catalog (LRP-406)                                                                                       |
| POST   | `/cases/{case_id}/letter-drafts`                                   | case_manager | Create staff-gated letter draft (optional `issue_source_id`); never auto-transmits                                                                              |
| GET    | `/cases/{case_id}/letter-drafts/{draft_id}`                        | read_only    | Get letter draft with sections, validation, and send guardrails                                                                                                 |
| PATCH  | `/cases/{case_id}/letter-drafts/{draft_id}/sections/{section_key}` | case_manager | Edit a draft section (versions history retained)                                                                                                                |
| POST   | `/cases/{case_id}/letter-drafts/{draft_id}/validate`               | case_manager | Re-run claim-safe validation checklist                                                                                                                          |
| POST   | `/cases/{case_id}/letter-drafts/{draft_id}/advance`                | case_manager | Advance staff workflow (transmission statuses blocked — use mark-sent)                                                                                          |
| POST   | `/cases/{case_id}/letter-drafts/{draft_id}/mark-sent`              | case_manager | Record that staff transmitted outside the platform; never mails/files                                                                                           |
...
| POST   | `/cases/{case_id}/dispute-strategy/prepare`                        | case_manager | Prepare CRA/furnisher letters from strategy stage (optional `account_keys`; lock-aware â€” identity-theft-locked tradelines are skipped into `locked`)          |
...
| GET    | `/clients/{client_id}/communication-preferences`                            | read_only    | Communication prefs + Do Not Call assistance (LRP-209; never silent registry submit)                                                                                                 |
| PUT    | `/clients/{client_id}/communication-preferences`                            | case_manager | Upsert communication preferences and DNC consent fields                                                                                                                              |
| POST   | `/clients/{client_id}/communication-preferences/do-not-call/open-registry`  | case_manager | Record official registry open + return donotcall.gov URL                                                                                                                             |
| POST   | `/clients/{client_id}/communication-preferences/do-not-call/mark-completed` | case_manager | Client/staff attestation that email confirmation finished; schedules 31-day follow-up                                                                                                |
| GET    | `/clients/{client_id}/unwanted-call-incidents`                              | read_only    | List unwanted-call incidents (LRP-209A; optional `status`; never auto-submitted)                                                                                                     |
| POST   | `/clients/{client_id}/unwanted-call-incidents`                              | case_manager | Log incident + preference snapshot + advisory eligibility + staff-gated draft                                                                                                        |
| GET    | `/clients/{client_id}/unwanted-call-incidents/{id}`                         | read_only    | Get unwanted-call incident detail                                                                                                                                                    |
| PATCH  | `/clients/{client_id}/unwanted-call-incidents/{id}`                         | case_manager | Update status/follow-up/external attestation/evidence; optional `refresh_draft`                                                                                                      |
| DELETE | `/clients/{client_id}/unwanted-call-incidents/{id}`                         | case_manager | Soft-delete unwanted-call incident                                                                                                                                                   |
...
| POST   | `/clients/{client_id}/portal-user`                                          | case_manager | Provision portal access + optional invite (LRP-301B; never emails a password)                                                                                                        |
| POST   | `/clients/{client_id}/portal-user/resend-invite`                            | case_manager | Resend one-time invite (rate-limited; LRP-301B)                                                                                                                                      |
...
| POST   | `/portal/auth/accept-invite`   | public     | Accept staff invite + set password; returns portal session (LRP-301B)     |
...
| GET    | `/accounts/reinvestigation-clock?case_id=`                       | read_only    | Per-account FCRA Â§611 reinvestigation clock for a case (computed)            |
...
| GET    | `/accounts/{account_id}/litigation-packet`                       | case_manager | Operator-gated Â§611/Â§623 litigation-readiness evidence packet (computed)    |
...
`GET /accounts/{account_id}/dispute-draft` returns a rule-based dispute draft for either a CRA tradeline investigation (`recipient_type=credit_bureau`, default) or a direct furnisher dispute (`recipient_type=furnisher`). The response includes structured `dispute_reason_suggestions`, `evidence_ready` / `missing_evidence` flags comparing the checklist to account and case fields, disputed item list, requested action, evidence checklist, and compliance notes for staff review. Legal references compare matched case FCRA findings and select the strongest deletion-oriented path (litigation-strength score, then deletion affinity preferring §605 obsolescence, then section coverage) plus the procedural dispute right (§611 CRA / §623 furnisher); when no matched finding exists, the procedural default is used. Response fields: `legal_citations`, `legal_pursuant`, `legal_reference_source` (`finding` | `default`), `legal_reference_rule_id`, and `legal_alternatives` (top compared options with `selected`, `deletion_affinity`, and rationale). Drafts are generated on demand and are not persisted in this foundation slice. Investigator aid only — not legal advice.
...
| POST   | `/documents/{document_id}/classify/reclassify` | case_manager | Enqueue async classification (OCR required)   |
...
`POST /documents/{document_id}/parsed-credit-report/reparse` enqueues `document_credit_report_parse` when the document has OCR text and `document_type=credit_report`. Returns 422 otherwise. Staff-mediated only; no live bureau contact. Supported layouts include Experian, Equifax, TransUnion (including ACR portal variants), and IdentityIQ / SmartCredit monitoring / tri-merge PDFs (parsers `identityiq` and `smartcredit`; empty bureau columns skipped; report-level bureau `unknown`; IdentityIQ golden PDF + expected JSON under `tests/fixtures/credit_reports/identityiq/`).
...
`POST /cases/{case_id}/resolutions/reresolve` enqueues `document_entity_resolve` for each case document with extracted metadata. Returns queued/skipped counts with skip reasons (`missing_metadata`, `enqueue_failed`). Soft-skips docs without extracted metadata. Returns 503 when entity resolution is disabled. Single-document `POST /documents/{id}/resolutions/reresolve` remains available. Staff-mediated only.
...
Requires `ENABLE_NATIVE_MOBILE_APP_STORE_DISTRIBUTION=true` (and native mobile passkey client readiness). Audit only â€” no App Store / Play Store release operations.
...
| Method | Path                                                                          | Min role              | Description                                                                                                                                                         |
| ------ | ----------------------------------------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/mortgage-partner/status`                                                    | case_manager          | Feature capabilities + deferred list (includes `partner_isolation_denial_suite`; LRP-501)                                                                           |
| GET    | `/mortgage-partner/automation-rules`                                          | case_manager          | List CRM automation rules; seeds org defaults when empty (LRP-203)                                                                                                  |
| POST   | `/mortgage-partner/automation-rules`                                          | admin                 | Create CRM automation rule (persisted; LRP-203)                                                                                                                     |
| PATCH  | `/mortgage-partner/automation-rules/{id}`                                     | admin                 | Update CRM automation rule (enable/disable + fields; LRP-203)                                                                                                       |
| POST   | `/mortgage-partner/automation-rules/{id}/fire`                                | admin                 | Staff-mediated dry-run/fire; durable audit event; live allowlist task/notification (LRP-502)                                                                        |
| GET    | `/mortgage-partner/automation-events`                                         | case_manager          | List CRM automation audit events; optional `rule_id` / `event_kind` (LRP-502)                                                                                       |
| GET    | `/mortgage-partner/automation-events/{id}`                                    | case_manager          | Get one CRM automation audit event (org-scoped; LRP-502)                                                                                                            |
| GET    | `/mortgage-partner/appointments`                                              | case_manager          | List CRM appointments (LRP-205)                                                                                                                                     |
| POST   | `/mortgage-partner/appointments`                                              | admin                 | Create appointment; fires matrix `consultation_scheduled` (LRP-205)                                                                                                 |
| PATCH  | `/mortgage-partner/appointments/{id}`                                         | admin                 | Update appointment status/schedule (LRP-205)                                                                                                                        |
| POST   | `/mortgage-partner/appointments/reminders/process`                            | admin                 | Process due T-24h / T-1h reminders via notification matrix (idempotent; LRP-205)                                                                                    |
| GET    | `/mortgage-partner/appointments/reminders`                                    | case_manager          | List appointment reminder audit runs (optional `?appointment_id=`; LRP-205)                                                                                         |
| GET    | `/mortgage-partner/nurture/programs`                                          | case_manager          | List nurture programs; seeds lender drip defaults when empty (LRP-206)                                                                                              |
| GET    | `/mortgage-partner/nurture/enrollments`                                       | case_manager          | List nurture enrollments for org (LRP-206)                                                                                                                          |
| POST   | `/mortgage-partner/nurture/enrollments`                                       | admin                 | Enroll contact (requires marketing opt-in; LRP-206)                                                                                                                 |
| PATCH  | `/mortgage-partner/nurture/enrollments/{id}`                                  | admin                 | Pause / resume / exit / opt-out enrollment (LRP-206)                                                                                                                |
| POST   | `/mortgage-partner/nurture/process`                                           | admin                 | Process due nurture steps (idempotent; consent/TCPA gated; LRP-206)                                                                                                 |
| GET    | `/mortgage-partner/nurture/deliveries`                                        | case_manager          | List nurture delivery audit runs (optional `?enrollment_id=`; LRP-206)                                                                                              |
| GET    | `/mortgage-partner/weekly-digests/subscriptions`                              | case_manager          | List weekly digest subscriptions (LRP-207)                                                                                                                          |
| POST   | `/mortgage-partner/weekly-digests/subscriptions`                              | admin                 | Create opt-in weekly digest subscription (LRP-207)                                                                                                                  |
| PATCH  | `/mortgage-partner/weekly-digests/subscriptions/{id}`                         | admin                 | Enable/disable / opt-out weekly digest subscription (LRP-207)                                                                                                       |
| POST   | `/mortgage-partner/weekly-digests/process`                                    | admin                 | Process weekly digests (idempotent per subscription+week; LRP-207)                                                                                                  |
| GET    | `/mortgage-partner/weekly-digests/runs`                                       | case_manager          | List archived weekly digest runs (optional `?partnership_id=`; LRP-207)                                                                                             |
| POST   | `/mortgage-partner/partnerships/{id}/realtor-invites`                         | admin                 | Create realtor invite (token returned once; realtor partnerships only; LRP-301)                                                                                     |
| POST   | `/mortgage-partner/partnerships/{id}/realtor-members/{mid}/disable`           | admin                 | Disable realtor membership (optional `?disable_user=true`; LRP-301)                                                                                                 |
| GET    | `/mortgage-partner/realtor/me`                                                | authenticated realtor | Active realtor session (partnership isolation; LRP-301)                                                                                                             |
| GET    | `/mortgage-partner/realtor/dashboard`                                         | authenticated realtor | Coarse stage summary + recent initials cards (LRP-302)                                                                                                              |
| GET    | `/mortgage-partner/realtor/referrals`                                         | authenticated realtor | Partnership-scoped PII-minimized referral list (LRP-302)                                                                                                            |
| GET    | `/mortgage-partner/realtor/pipeline`                                          | authenticated realtor | Coarse pipeline board for realtor partnership (LRP-302)                                                                                                             |
| GET    | `/mortgage-partner/realtor/invites/preview`                                   | public                | Invite preview for activation UI (`?token=`; LRP-301)                                                                                                               |
| POST   | `/mortgage-partner/realtor/invites/accept`                                    | public                | Accept invite + set password; returns staff JWT + realtor session (LRP-301)                                                                                         |
| POST   | `/mortgage-partner/realtor/password-reset/request`                            | public                | Request password reset (generic response; reset token echoed in development/test only; LRP-301)                                                                     |
| POST   | `/mortgage-partner/realtor/password-reset/confirm`                            | public                | Confirm password reset; returns staff JWT + realtor session (LRP-301)                                                                                               |
| GET    | `/mortgage-partner/referral-intake/status`                                    | public                | Referral web-form readiness (LRP-103; no auth)                                                                                                                      |
| POST   | `/mortgage-partner/referral-intake`                                           | public                | Web-form intake: creates client + case + referral + ops task; runs post-accept orchestrator (LRP-103/201); quarantines SSN in free text                             |
| GET    | `/mortgage-partner/referral-intake/{intake_id}/orchestrator`                  | case_manager          | Post-accept orchestrator audit (assignment + notify drafts; LRP-201)                                                                                                |
...
| GET    | `/mortgage-partner/partnerships`                                              | case_manager          | List partnerships for caller org (includes `primary_contact_*`, `active_referral_count`)                                                                            |
| GET    | `/mortgage-partner/partnerships/{id}`                                         | case_manager          | Get partnership (audited; includes CRM enrichment fields)                                                                                                           |
...
| GET    | `/mortgage-partner/partnerships/{id}/contacts`                                | case_manager          | List CRM partner contacts (audited; LRP-101)                                                                                                                        |
| POST   | `/mortgage-partner/partnerships/{id}/contacts`                                | admin                 | Create CRM partner contact; `is_primary` clears other primaries (audited)                                                                                           |
| PATCH  | `/mortgage-partner/partnerships/{id}/contacts/{cid}`                          | admin                 | Update CRM partner contact (audited)                                                                                                                                |
...
| GET    | `/mortgage-partner/partnerships/{id}/referrals/{rid}/readiness-report`        | case_manager          | Advisory readiness report (JSON) for a referral; resolves latest published credit_analysis_run; 404 if no case_id or no published run; disclaimer on every response |
| GET    | `/mortgage-partner/partnerships/{id}/referrals/{rid}/readiness-report/export` | case_manager          | Export readiness report as text or PDF (`?format=text\|pdf`); disclaimer leads every export; never auto-transmitted                                                 |
| GET    | `/mortgage-partner/partnerships/{id}/readiness-reports`                       | case_manager          | List advisory readiness summaries for all referrals with published runs in this partnership                                                                         |
...
| GET    | `/org-admin/dispute-settings`           | admin    | Org dispute settings (tolerance + org/bureau/recipient windows) |
...
Requires `ENABLE_PUBLIC_OAUTH_MARKETPLACE_LISTINGS=true` (and OAuth marketplace publishing readiness). Audit only â€” no unreviewed third-party auto-approve.
...
Requires `ENABLE_UNSUPERVISED_AUTONOMOUS_FILING_LOOPS=true` (and fully autonomous bureau API filing readiness). No live bureau HTTP calls â€” timeline audit only.
...
| GET    | `/reporting/reinvestigation-outcomes/benchmarks/export` | read_only | Org-internal benchmark rates CSV export (counts/rates only; no PII) |
| GET    | `/reporting/revenue`                                    | read_only | Org revenue readiness from Stripe billing state                     |

`GET /reporting/reinvestigation-outcomes` (Phase 11) returns an org-scoped analytics read model over recorded dispute responses. `analytics` contains `total_responses`, per-outcome `counts` (`deleted`/`verified`/`updated`/`corrected`/`no_response`/`rejected`), and the derived rates `deletion_rate`, `verification_rate`, `correction_rate`, `favorable_rate` (deleted + corrected), and `no_response_rate` (each a fraction of total responses). Time-to-response is measured from the linked sent letter's `sent_at` (falling back to the account `last_dispute_date`) to the response date: `avg_days_to_response`, `median_days_to_response`, and `measured_response_count` cover only substantive responses (a recorded `no_response` has no elapsed time). As of Phase 12 the endpoint accepts optional `start` / `end` (filter by response day, inclusive) and `bureau` (single credit bureau) query params; the applied filters are echoed back under `filters` (`start`/`end`/`bureau`/`group_by`, each `null` when unset). As of Phase 13, optional `group_by=bureau` returns a `by_bureau` array of `{bureau, analytics}` roll-ups so operators can compare all bureaus in one call (the top-level `analytics` aggregate is unchanged). As of Phase 14, optional `group_by=recipient` returns a `by_recipient` array of `{recipient, analytics}` roll-ups (credit bureau vs furnisher, attributed via the linked dispute letter; unlinked responses are bucketed as `unknown`); other `group_by` values return `422`. Purely computed over stored data â€” org-scoped only (no cross-tenant benchmarks) and no live bureau contact.

`GET /reporting/reinvestigation-outcomes/benchmarks` (Phase 15) returns org-internal trailing baselines for the same analytics shape. Query params: `baseline_days` (range 7â€“365), `recent_days` (must be â‰¤ `baseline_days`), optional `bureau`, optional `recipient` (`credit_bureau` / `furnisher`, Phase 20), and optional `group_by` of `bureau` (Phase 18) or `recipient` (Phase 19). When window params are omitted, dispute-settings defaults apply: per-recipient override when `recipient` is set (Phase 20), else per-bureau override when `bureau` is set (Phase 18), else org-wide pair (Phase 17), else platform `90` / `30`. Response includes `scope` (`organization`), `baseline_period` / `baseline`, `recent_period` / `recent`, advisory `rate_deltas`, and when `group_by=bureau` a `by_bureau` array of per-bureau baseline/recent analytics + deltas (or when `group_by=recipient` a `by_recipient` array attributed via the linked dispute letter; unlinked responses are `unknown`). No cross-tenant data and no live bureau contact; invalid `group_by` or `recent_days` > `baseline_days` returns `422`. As of Phase 16 the Enterprise reporting UI exposes an **Outcome benchmarks** tab; Phase 17 initializes from org defaults; Phase 18 adds per-bureau window overrides; Phase 19 adds a Bureau/Recipient breakdown control matching outcome analytics.

`GET /reporting/reinvestigation-outcomes/benchmarks/export?format=csv` (Phase 19) downloads the same org-scoped benchmark windows and aggregate rates as CSV for operator handoff. Accepts the same optional window, `bureau`, and `group_by` query params as the JSON benchmarks endpoint. Payload is counts and rates only â€” no client/account identifiers or response-level rows. Returns `422` when `format` is not `csv`.
...
counts, and a heuristic `outcome_score` (0â€“100). Returns `404` when predictive analytics is
disabled.
...
| Method | Path                                                                          | Min role  | Description                                               |
| ------ | ----------------------------------------------------------------------------- | --------- | --------------------------------------------------------- |
| GET    | `/reporting/unredacted-cross-org-benchmark-exports/status`                    | â€”       | Export readiness and blockers                             |
...
`content_type`, and `byte_size` only â€” no raw tenant PII dump.

| Method | Path                                                                                  | Min role  | Description                                            |
| ------ | ------------------------------------------------------------------------------------- | --------- | ------------------------------------------------------ |
| GET    | `/reporting/live-unredacted-benchmark-blob-exports/status`                            | â€”       | Blob export readiness and blockers                     |
