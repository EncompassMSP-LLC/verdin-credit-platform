...
`GET /accounts/reinvestigation-clock?case_id={case_id}` (Phase 10) returns a computed FCRA §611 reinvestigation clock for every account in a case. Each entry classifies the tradeline by `state` — `not_sent` (no dispute mailed), `awaiting`, `due_soon` (within 7 days of the deadline), `overdue` (30-day window elapsed with no recorded response), or `responded` — and includes the `deadline` (`clock_start_date` + 30 days), `days_remaining`, `response_received`, and `response_count`. As of Phase 11 the clock keys off the latest actually-sent `dispute_letters.sent_at` (the newest mailed round): `clock_start_date` is that sent date (falling back to the account `last_dispute_date` when no letter is sent), and `dispute_round_count` reports how many dispute letters have been sent for the tradeline. Entries are sorted overdue → due-soon → awaiting → responded → not-sent, and a `summary` gives per-state counts. Purely a read model over stored dispute dates, sent letters, and recorded responses (`no_response` outcomes do not count as `responded`); no live bureau contact.
