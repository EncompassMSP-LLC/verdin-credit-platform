...
Ordered path to **pilot-ready product surfaces** — **complete**. Preceded by shipped **v5.0.0**.

Linked from [`docs/roadmap/README.md`](../roadmap/README.md).

## Exit criteria for “5.0+ pilot done”

- [x] All eight epics below are **✅ or explicitly deferred** with docs updated — see [`version-5.0-plus-scope.md`](../governance/version-5.0-plus-scope.md)
- [x] Capability matrix **Frontend** column updated for 5.0+ UI slices
- [x] Staff can manage clients and link cases without API calls
- [x] Portal users can upload documents and view/send messages on linked cases (when flags enabled)
- [x] `pnpm dev` in `apps/web` works without manual `api-client` build
