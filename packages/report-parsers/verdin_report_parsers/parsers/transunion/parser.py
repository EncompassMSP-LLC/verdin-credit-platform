"""TransUnion report parser (stub — registered when layout rules are implemented)."""
...
class TransUnionParser:
    ...
    def can_parse(self, document: ParsedDocument) -> float:
        if detect_bureau(document.searchable_text()) != Bureau.TRANSUNION:
            return 0.0
        return 0.0

    def parse(self, document: ParsedDocument) -> ParsedCreditReport:
        raise NotImplementedError("TransUnion layout parsing is not implemented yet")
