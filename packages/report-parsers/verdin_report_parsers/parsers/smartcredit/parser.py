"""Production SmartCredit monitoring / tri-merge credit report parser."""

from verdin_report_parsers.base import ParsedDocument
from verdin_report_parsers.constants import Bureau
from verdin_report_parsers.models import ParsedCreditReport, ParseMetadata

# SmartCredit tri-merge field layout matches the IdentityIQ column expansion model.
from verdin_report_parsers.parsers.identityiq.extract import (
    build_summary,
    extract_accounts,
    extract_collections,
    extract_consumer,
...
from verdin_report_parsers.parsers.smartcredit.layout import score_layout, split_sections

PARSER_VERSION = "1.0.0"


class SmartCreditParser:
    """Parse SmartCredit member credit-report PDFs into ``ParsedCreditReport``.

    Tri-merge layouts expand each bureau column into a separate tradeline so
    cross-bureau comparison can run. Report-level ``bureau`` is ``unknown``.
    """
    ...
    def parse(self, document: ParsedDocument) -> ParsedCreditReport:
        ...
        consumer, consumer_confidence = extract_consumer(
            sections.get("personal_information", ""),
            source_text,
        )
        ...
        required_sections = ("personal_information", "accounts")
