"""IdentityIQ monitoring / tri-merge credit report layout detection."""
...
_IDENTITYIQ_BRANDING = re.compile(
    r"\bidentity\s*iq\b|member\.identityiq\.com|credit\s+report\s*-\s*identityiq",
    re.I,
)
_PORTAL_URL = re.compile(r"member\.identityiq\.com(?:/CreditReport\.aspx)?", re.I)
...
_REPORT_HEADER = re.compile(r"credit\s+report\s*-\s*identityiq|3[- ]bureau\s+credit\s+report", re.I)
...
def is_identityiq_layout(text: str) -> bool:
    return bool(_IDENTITYIQ_BRANDING.search(text))


def score_layout(text: str) -> LayoutScore:
    """Return deterministic layout confidence for IdentityIQ monitoring reports."""
    if not _IDENTITYIQ_BRANDING.search(text):
