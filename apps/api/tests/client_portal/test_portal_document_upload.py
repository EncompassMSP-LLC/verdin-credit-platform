...
def test_portal_user_uploads_proof_of_address_to_linked_case(
    api_client: TestClient,
    manager_headers: dict[str, str],
    portal_enabled: None,
    memory_storage: MemoryDocumentStorage,
) -> None:
    email = f"portal-address-upload-{uuid.uuid4().hex[:8]}@example.com"
    client_id = _create_client(
        api_client,
        manager_headers,
        display_name=f"Address Upload Client {uuid.uuid4().hex[:6]}",
        email=email,
    )
    _provision_portal_user(api_client, manager_headers, client_id, email=email)
    linked_case = _create_case(
    ...
    filename, file_obj, content_type = sample_pdf_upload()
    upload_response = api_client.post(
        f"/api/v1/portal/cases/{linked_case['id']}/proof-of-address-document",
        headers=portal_headers,
        files={"file": (filename, file_obj, content_type)},
    )
    assert upload_response.status_code == 201, upload_response.text
    uploaded = upload_response.json()
    assert uploaded["title"] == "Proof of mailing address"
    assert uploaded["document_type"] == "proof_of_address"
    assert uploaded["case_id"] == linked_case["id"]
    assert len(memory_storage._objects) == 1
    ...
    assert list_response.status_code == 200, list_response.text
    body = list_response.json()
    assert body["proof_of_address_on_file"] is True
    assert body["proof_of_address_document_id"] == uploaded["id"]
    ...
    assert case_response.status_code == 200, case_response.text
    assert case_response.json()["proof_of_address_document_id"] == uploaded["id"]
