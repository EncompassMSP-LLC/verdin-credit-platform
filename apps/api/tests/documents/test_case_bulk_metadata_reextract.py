"""Case-level bulk metadata re-extract tests."""
...
def _fake_enqueue(job_type: JobType, payload: dict | None = None) -> JobMessage:
    return JobMessage(
        job_type=job_type,
        payload=payload or {},
        job_id=f"bulk-meta-{payload.get('document_id', 'unknown') if payload else 'unknown'}",
    )
...
def test_bulk_reextract_metadata_queues_eligible_and_skips_missing_ocr(
    mock_enqueue,
    api_client: TestClient,
    manager_headers: dict[str, str],
    ...
    response = api_client.post(
        f"/api/v1/cases/{case_id}/metadata/reextract",
        headers=manager_headers,
    )
