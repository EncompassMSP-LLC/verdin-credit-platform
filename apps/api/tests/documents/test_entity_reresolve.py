"""Operator async document entity re-resolve enqueue tests."""
...
async def document_with_ocr(
    ...
    create = api_client.post(
        ...
        data={"title": "Entity Reresolve Test", "case_id": sample_case_id},
...
def document_with_metadata(
    ...
    return document_with_ocr


@patch("api.modules.documents.service.enqueue_job", side_effect=_fake_enqueue)
def test_reresolve_enqueues_job(
    mock_enqueue,
    api_client: TestClient,
    manager_headers: dict[str, str],
    document_with_metadata: str,
) -> None:
    response = api_client.post(
        f"/api/v1/documents/{document_with_metadata}/resolutions/reresolve",
        headers=manager_headers,
    )
    assert response.status_code == 200, response.text
    data = response.json()
    assert data["document_id"] == document_with_metadata
    assert data["job_id"] == "test-reresolve-job"
    assert data["job_type"] == JobType.DOCUMENT_ENTITY_RESOLVE.value
    assert data["queued"] is True
    mock_enqueue.assert_any_call(
        JobType.DOCUMENT_ENTITY_RESOLVE,
        {"document_id": document_with_metadata},
    )


def test_reresolve_requires_extracted_metadata(
    api_client: TestClient,
    manager_headers: dict[str, str],
    document_with_ocr: str,
) -> None:
    response = api_client.post(
        f"/api/v1/documents/{document_with_ocr}/resolutions/reresolve",
        headers=manager_headers,
    )
    assert response.status_code == 422
    assert "Metadata must be extracted" in response.json()["detail"]


@patch("api.modules.documents.service.enqueue_job", side_effect=_fake_enqueue)
def test_reresolve_returns_503_when_disabled(
    _mock_enqueue,
    api_client: TestClient,
    manager_headers: dict[str, str],
    document_with_metadata: str,
