"""Admin-gated agent unsupervised loop run models."""
...
class AgentUnsupervisedLoopStatus(StrEnum):
...
class AgentUnsupervisedLoopRun(Base, TimestampMixin):
    __tablename__ = "agent_unsupervised_loop_runs"
    ...
    agent_supervised_loop_run_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("agent_supervised_loop_runs.id"),
        nullable=False,
        index=True,
    )
    ...
    status: Mapped[AgentUnsupervisedLoopStatus] = mapped_column(
        Enum(
