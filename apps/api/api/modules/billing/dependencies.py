...
def require_stripe_live_tax_api_enabled() -> None:
    require_stripe_tax_calculation_enabled()
    if not is_feature_enabled(FeatureFlag.ENABLE_STRIPE_LIVE_TAX_API):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Stripe live Tax API is not enabled",
        )
