...
class DocumentService:
    ...
    async def upload_identity_document_for_portal(
        self,
        *,
        organization_id: uuid.UUID,
        portal_user_id: uuid.UUID,
        ...
        """Upload a government ID from the borrower portal and link case/client pointers."""
        from api.modules.clients.models import Client
        from api.modules.documents.constants import DocumentType
        ...
        document = await self.upload_document_for_portal(
            ...
            case_id=case_id,
            file=file,
            title=(title or "").strip() or "Driver's license",
            description="Government-issued photo ID for dispute mail packets",
        )
    ...
    async def bulk_reextract_case_metadata(
        ...
        """Enqueue metadata extract for each OCR'd document on a case."""
        self._require_write(user)
        organization_id = self._require_organization(user)
        await self._validate_case(case_id, organization_id)
        ...
        for document in documents:
            ...
            try:
                ...
                logger.exception(
                    "Failed to enqueue metadata extract for document %s",
                    document.id,
                )
                skipped.append(
                    CaseMetadataReextractSkippedItem(
    ...
    async def export_case_cfpb_checklist_packet(
        ...
        *,
        ...
        include_report_excerpts: bool = False,
    ) -> tuple[bytes, str, str]:
    ...
    async def _export_case_checklist_packet(
        ...
        *,
        ...
        if self._session is not None:
            ...
            if include_report_excerpts:
                from api.modules.accounts.service import AccountService

                account_service = AccountService.from_session(self._session)
                for filename, content in await account_service.collect_case_report_excerpt_files(
                    user,
                    case_id,
                    require_consents=True,
                ):
                    exhibits.append((report_excerpt_exhibit_path(filename), content))
