...
interface DocumentLlmSummaryPanelProps {
  documentId: string;
}

export function DocumentLlmSummaryPanel({ documentId }: DocumentLlmSummaryPanelProps) {
  const [summary, setSummary] = useState<DocumentLlmSummary | null>(null);
  ...
  const generateMutation = useMutation({
    mutationFn: () => generateDocumentLlmSummary(documentId),
    onSuccess: (result) => {
      setSummary(result);
      setError(null);
    },
  ...
  return (
    <Card title="AI document summary" className="lg:col-span-3">
      <p className="text-sm text-gray-600">
        Generate a PII-scrubbed document summary using OCR text and extracted metadata.
      </p>
