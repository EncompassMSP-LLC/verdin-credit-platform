...
import { portalAcceptInvite } from '@verdin/api-client';
...
export function PortalAcceptInvitePage() {
  ...
  const onSubmit = async (data: AcceptForm) => {
    setError(null);
    try {
      const tokens = await portalAcceptInvite({
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-8 shadow-sm">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Use the invite link from your readiness partner to choose a password. No temporary
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-4">
          ...
          <Button type="submit" className="w-full" disabled={isSubmitting}>
            {isSubmitting ? 'Activating…' : 'Set password and continue'}
          </Button>
        </form>
