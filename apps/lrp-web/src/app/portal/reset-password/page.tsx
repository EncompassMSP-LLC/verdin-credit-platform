...
import { ApiClientError, configureApiClient, portalResetPassword } from '@verdin/api-client';
...
function ResetPasswordForm() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const tokens = await portalResetPassword({ token: token.trim(), password });
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4" noValidate>
      ...
      <p className="text-sm text-slate-500 dark:text-white/65">
        Paste the reset token from your email (or the development link) and set a new password.
      </p>
      <div>
...
export default function ResetPasswordPage() {
  return (
    <AuthShell title="New password" subtitle="Complete your borrower portal password reset.">
