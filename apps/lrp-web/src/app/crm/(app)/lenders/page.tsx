'use client';

import { DataTable } from '@/components/lender/DataTable';
import { PageHeader } from '@/components/crm/PageHeader';
import { RoleGate } from '@/components/crm/RoleGate';
import { lenders } from '@/lib/crm/data';

export default function CrmLendersPage() {
  return (
    <RoleGate
      permission="lenders.view"
      fallback={<p className="text-sm text-slate-500">No access to lenders.</p>}
    >
      <PageHeader
        eyebrow="Relationships"
        title="Lenders"
        description="Loan officer directory with NMLS, market, and open referral counts."
      />
      <div className="rounded-md border border-navy-900/10 bg-white dark:border-white/10 dark:bg-navy-800">
        <DataTable
          rows={lenders}
          columns={[
            {
              key: 'name',
              header: 'Loan officer',
              cell: (r) => (
                <div>
                  <p className="font-medium">{r.displayName}</p>
                  <p className="text-xs text-slate-500">{r.title}</p>
                </div>
              ),
            },
            { key: 'org', header: 'Organization', cell: (r) => r.organizationName },
            { key: 'nmls', header: 'NMLS', cell: (r) => r.nmls ?? '—' },
            { key: 'market', header: 'Market', cell: (r) => r.market },
            {
              key: 'borrowers',
              header: 'Active borrowers',
              cell: (r) => String(r.activeBorrowers),
            },
            {
              key: 'refs',
              header: 'Open referrals',
              cell: (r) => String(r.openReferrals),
            },
            { key: 'status', header: 'Status', cell: (r) => r.status },
          ]}
        />
      </div>
    </RoleGate>
  );
}
